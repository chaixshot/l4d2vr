...
public:
	...
	virtual unsigned long long GetUint64(const char* keyName = nullptr, unsigned long long defaultValue = 0) const = 0;
	...
	virtual bool ForEventData(void* eventVisitor) const = 0;
};
...
// Minimal Source tier1 functor/call-queue ABI used by queued material rendering.
// The concrete queue is owned by Source. Plugin functors are AddRef'd when queued,
// invoked by the material worker after earlier render commands, then Released.
class CFunctor : public IRefCounted
{
public:
	virtual ~CFunctor() {}
	virtual void operator()() = 0;
	unsigned int m_nUserID = 0;
};

class ICallQueue
{
public:
	void QueueFunctor(CFunctor* functor)
	{
		if (!functor)
			return;

		functor->AddRef();
		QueueFunctorInternal(functor);
	}

private:
	virtual void QueueFunctorInternal(CFunctor* functor) = 0;
};
...
public:
	...
	// L4D2 local materialsystem.dll: absolute vtable slot 142 returns this+0x2A4,
	// the embedded CCallQueueExternal<CMatQueuedRenderContext>. IMatRenderContext
	// inherits IRefCounted, so this is declaration slot 140 here.
