...
void VR::SubmitVRTextures()
{
    ...
    // writing a new stereo pair. Submitting the old eye textures while the HMD moved is
    // the most visible form of multicore ghosting, so wait briefly for the render-thread
    // handoff before falling back to the previous frame.
    if (queued && inGame)
    {
